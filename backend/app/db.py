...
async def init_db() -> None:
    ...
    for q in SEED_CODING_BANK:
        await coding_bank_collection.update_one(
            {"title": q["title"]},
            {"$set": q},
            upsert=True,
        )
    logger.info(f"Seeded/updated {len(SEED_CODING_BANK)} coding questions into coding_bank with multi-language starter code.")
